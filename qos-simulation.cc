/* =============================================================================
 * QoS Simulation V11 — Version améliorée
 * Projet ISIC 2A — Prof. HAIDINE — Groupe B4
 *
 * AMÉLIORATIONS V11 vs V10 :
 * ─────────────────────────────────────────────────────────────────────────────
 * [A] MODÈLE DE PROPAGATION RÉALISTE
 *     - Scénario Wi-Fi : ThreeLogDistancePropagationLossModel remplace Default()
 *       => pertes radio variables selon distance, plus proches du terrain réel
 *     - Scénario LTE   : Hybrid Buildings propagation model (Indoor/Outdoor)
 *       => différencie les UE proches (bon SINR) des UE lointains (dégradés)
 *
 * [B] TRAFIC BACKGROUND PLUS AGRESSIF ET RÉALISTE
 *     - Trafic BG utilise OnOffHelper avec distribution ON/OFF exponentielle
 *       (mean ON = 0.5s, mean OFF = 0.1s) => rafales imprévisibles, plus réaliste
 *       qu'un flux constant, crée des pics de congestion transitoires
 *     - SC1 Wi-Fi Statique : 3 flux BG (au lieu de 2) pour saturer à ~2.1Mbps
 *     - SC3 LTE            : backhaul 1.5Mbps (au lieu de 2Mbps) pour forcer
 *       la saturation même avec un BG légèrement réduit => différence QoS nette
 *
 * [C] MARQUAGE DSCP COMPLET ET COHÉRENT
 *     - TOS=0xB8 (EF, DSCP 46)  => VoIP  dans toutes les fonctions
 *     - TOS=0x88 (AF41, DSCP 34) => Vidéo dans toutes les fonctions
 *     - TOS=0x08 (CS1, DSCP 8)   => BG    (au lieu de 0x20, plus standard)
 *     - La PrioQueueDisc utilise une Priomap alignée sur ces valeurs DSCP
 *       Bande 0 (haute prio) : DSCP EF  (TOS >> 2 = 46 => précédence 5 → band 0)
 *       Bande 1 (moy. prio)  : DSCP AF41 (précédence 4 → band 1)
 *       Bande 2 (faible prio): BE / CS1 (précédences 0-3 → band 2)
 *     - Priomap corrigée : "0 0 0 0 0 1 1 2 2 2 2 2 2 2 2 2"
 *       (index = IP Precedence 0..15, valeur = bande 0/1/2)
 *
 * [D] QUEUE DISCIPLINE AMÉLIORÉE
 *     - Bande haute priorité : RED (Random Early Detection) au lieu de FIFO
 *       => évite la synchronisation TCP et lisse les délais pour VoIP/Vidéo
 *     - Bandes moyennes/basses : FIFO conservé
 *     - Taille de queue réduite à 4p pour la bande haute prio
 *       => drops rapides sur BG, protection accrue pour VoIP
 *
 * [E] MOBILITÉ PLUS RÉALISTE (Scénario 2)
 *     - Modèle RandomWaypointMobilityModel pour la STA mobile
 *       avec pause aléatoire [0,2s] et vitesse [0.5, 1.5] m/s
 *       => handover non déterministe, plus proche d'un déplacement piéton
 *     - Position initiale proche AP1, dérive naturellement vers AP2
 *
 * [F] MÉTRIQUES SUPPLÉMENTAIRES
 *     - Calcul du MOS (Mean Opinion Score) pour VoIP via formule E-Model simplifiée
 *       MOS = 1 + 0.035*R + R*(R-60)*(100-R)*7e-6  avec R = 93.2 - Id - Ie
 *     - Calcul du throughput utile (goodput) séparé du débit brut
 *     - Export CSV des résultats pour post-traitement (gnuplot / Python)
 *
 * [G] RÉPÉTITIONS STATISTIQUES
 *     - RunXxx() accepte un paramètre runId (seed RNG) pour permettre
 *       plusieurs runs avec des graines différentes
 *     - main() fait 3 runs par scénario et affiche la moyenne ± écart-type
 *       => résultats statistiquement significatifs, attendus en simulation
 *
 * RÉFÉRENCES :
 *   [1] R. Jain et al., DEC TR-301, 1984 — Indice de Jain
 *   [2] IEEE 802.11e-2005 — WMM / EDCA
 *   [3] 3GPP TS 23.203 / TS 23.501 — QCI, EPS Bearers, seuils QoS
 *   [4] RFC 2474 (DSCP), RFC 3246 (EF), RFC 2597 (AF)
 *   [5] ITU-T G.107 — E-Model pour MOS VoIP
 * =============================================================================
 */
